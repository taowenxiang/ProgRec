# ProgRec AI Agent Design

**Date:** 2026-05-11

## Goal

Upgrade the current `progrec_agent` from a fixed CLI orchestration wrapper into a true AI agent that plans over the existing five ProgRec skills, interacts through natural language, and adaptively re-runs recommendation steps when user goals or recommendation quality require it.
...
The current `progrec_agent/` package wraps these skills into an interactive CLI and batch runner, but it is still fundamentally a deterministic pipeline:
...
The new ProgRec system should be framed as:
...
The new agent behavior should be introduced inside `progrec_agent/` without destabilizing the existing skill trees.
...
`progrec_agent/session.py` should persist enough information for multi-turn agent behavior.
...
Add the new `progrec_agent/` modules and refactor the current flow into:
...
**ProgRec is no longer just a recommendation pipeline. It is an AI agent that understands user goals, plans over five specialized SNA skills, adapts recommendation strategy interactively, and explains its choices.**
