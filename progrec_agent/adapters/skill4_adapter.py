...
_SKILL4_ROOT = Path(__file__).resolve().parents[2] / "skill4_program_teammate_discovery"
...
from progrec_agent.config import ResourceConfig, skill1_profiles_path


def _first_existing_academic_graph(repo_root: Path) -> str:
    """Match ``skill4_program_teammate_discovery/main.py`` auto-resolve order so dataset runs see the real graph."""
    for candidate in (
    ...
    *,
    ...
    if bundle is not None:
        ...
        skill1_s = str(bundle.skill1_profiles) if bundle.skill1_profiles else str(
            skill1_profiles_path(repo_root)
        )
    else:
        graph_s = _first_existing_academic_graph(repo_root)
        students_s = ""
        mentors_s = ""
        skill1_s = str(skill1_profiles_path(repo_root))
...
def run_skill4_custom_mode(
    *,
    ...
    students_path = repo_root / "skill2_academic_graph_builder/outputs/student_profiles_standard.json"
