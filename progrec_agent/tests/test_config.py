"""Tests for progrec_agent.config."""
...
class TestConfig(unittest.TestCase):
    def test_resolve_repo_root_default(self) -> None:
        ...
        self.assertTrue((root / "skill1_student_profiling").is_dir())
        self.assertTrue((root / "progrec_agent" / "config.py").is_file())
    ...
    def test_demo_config_resolves(self) -> None:
        ...
        self.assertEqual(
            cfg.skill1_profiles,
            root / "skill1_student_profiling" / "outputs" / "student_profiles_normalized.jsonl",
        )
    ...
    def test_graph_config_raises_when_graph_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            fake = Path(td) / "repo"
            (fake / "skill1_student_profiling" / "outputs").mkdir(parents=True)
            proc = fake / "skill2_academic_graph_builder" / "regenerate_kit" / "data" / "processed"
    ...
    def test_graph_config_ok_when_bundle_exists(self) -> None:
        root = _real_repo()
        graph = root / "skill2_academic_graph_builder/regenerate_kit/data/processed/academic_graph.json"
