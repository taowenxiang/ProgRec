#!/usr/bin/env python3
"""Non-interactive StuRec Agent — batch pipeline (Skills 3 → 4 → 5).

Uses :mod:`sturec_agent.config` for demo vs graph paths and :mod:`sturec_agent.schemas`
for preflight / post-run checks. Example::

    python3 sturec_agent/run_agent.py --mode graph --student-id jamie-taylor-00008 --output outputs/final.json
"""
...
from sturec_agent.config import ResourceConfig, resolve_repo_root, resolve_resource_config
from sturec_agent.schemas import (
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run StuRec pipeline (Skill 3 → 4 → 5) for one student_id.",
    )
    p.add_argument(
        "--student-id",
        default=None,
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override repository root (default: ancestor of sturec_agent/ with skill1_handoff/).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.work_dir is not None:
        ...
        work = Path(tempfile.mkdtemp(prefix="sturec_run_"))
        cleanup = not args.keep_work_dir

    orchestrator = StuRecOrchestrator(repo_root=repo_root, temp_dir=work)
