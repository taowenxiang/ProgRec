...
class TestReplAgentFlow(unittest.TestCase):
    ...
    @patch("sturec_agent.repl.build_strategy", return_value={"top_k": 5, "max_reruns": 0})
    @patch("sturec_agent.repl.judge_results", return_value={"rerun_needed": False, "reasons": [], "stop_reason": "quality acceptable"})
    @patch("sturec_agent.repl.build_execution_plan")
    @patch("sturec_agent.repl.build_profiles_from_text")
    @patch("sturec_agent.repl._build_llm_client_from_env", return_value=object())
